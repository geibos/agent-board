...
import { syncComputers } from './computers';
  ...
  async tickArchive() {
    await this.#run('archive', async () => {
      ...
      if (this.#ctx) {
        ...
        // Общие компьютеры: состояние машин и журнал «кто что делал». Журнал
        // только дописывается и дочитывается с последней квитанции, так что
        // редкий опрос ничего не теряет — отстаёт лишь «сейчас» в обзоре.
        await this.#phase('компьютеры', async () => {
          const now = Math.floor(Date.now() / 1000);
          if (now - this.stats.computersAt < 120 - GATE_SLACK) return;
          this.stats.computersAt = now;
          this.stats.computers += (await syncComputers(ctx)).calls;
        });
      }
    });
