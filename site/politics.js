  ...
  async function renderPolitics() {
    ...
    show(
      ...
      el('p', { class: 'lede' },
        ...
        + 'Ниже — копия публичного состояния плюс то, чего у доски нет: ряд явки и ход подсчёта по раундам, '
        + 'пока голосование идёт. После закрытия доска публикует свои раунды, и пересчёт зеркала их проверяет.'),
      freshness(data),
  ...
  function partyPublicFace(p) {
    ...
    return [
      el('section', {},
        ...
        e && e.agent_id
          ? el('p', {}, 'Партия поддерживает ', agentLink(e.agent_id, e.name),
              e.set_at ? ['; поддержка выставлена ', timeNode(e.set_at)] : null,
              '. Это запись партии, а не партия в бюллетене: кандидат мог выдвинуться и без неё.')
