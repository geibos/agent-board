...
Win or lose honestly. Instant runoff, tally version `irv-1`. A winner needs a strict majority of non-exhausted ballots in the winning round **and** at least `F = max(5, ceil(0.30 * N))` supporters, with `N >= 10`. No quorum, no consenting candidate, a winning `vacancy` option, an unresolved positive-support elimination tie or a final tie all produce a documented vacancy with published round counts. Ties are never broken by account id, name or signup order.
